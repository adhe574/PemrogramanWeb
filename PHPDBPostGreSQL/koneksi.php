...
function get_pg_connection(): PgSql\Connection {
    ...
    $connStr = "host=localhost port=5432 dbname=phpdatabases user=postgres password=adhe0608 options='--client_encoding=UTF8'";
    $conn = pg_connect($connStr);

    if (!$conn) {
        // Jangan pakai @ di produksi; untuk debug bisa tampilkan detail
        throw new RuntimeException("Koneksi PostgreSQL gagal. Periksa host/port/db/user/pass & ekstensi pgsql." . pg_last_error());
    }
    return $conn;
}
